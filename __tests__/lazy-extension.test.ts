...
import { BLOCKING_EVENTS, REPLAY_EVENTS, STARTUP_EVENTS, installDeferred } from "../lazy-extension.ts";
...
register("installs the real runtime module through the real deferred loader", { timeout: 120000 }, async () => {
	...
	const realLoad: DeferredLoad = async () => import("../index.ts");
...
register("real bootstrap declares matching capabilities and loads the real factory", { timeout: 120000 }, async () => {
	...
	const bootstrap = (await import("../bootstrap.ts")).default as (pi: DeferredPi) => unknown;
	await bootstrap(pi);
	for (const name of ["mcp","mcp-auth"]) {
	...
	const commandIdentity = new Map(["mcp","mcp-auth"].map((name) => [name, commands.get(name)]));
	await emit("tool_call");
	const realigned = ["mcp","mcp-auth"].some((name) => commands.get(name) !== commandIdentity.get(name));
	assert.ok(
